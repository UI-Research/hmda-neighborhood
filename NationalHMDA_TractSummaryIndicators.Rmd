...
nathmda_comb <- rowwise(nathmda_flags) %>% 
  mutate(#Top-line indicators
         ...
         medincome = if_else(std_flag == 1, actualincome, NA_integer_),
         medloanamount = if_else(std_flag == 1, loan_amount, NA_integer_),
         )

skim(nathmda_flags %>% select(actualincome, loan_amount))
skim(nathmda_comb %>% select(medincome, medloanamount))

nathmda_comb %>% select(medincome, medloanamount) %>% skim
...
colnames(hmda_final) %>% str_detect("invest")

nathmda_in %>% select(income) %>% skim
