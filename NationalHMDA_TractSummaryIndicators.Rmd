...
## histogram of co\applicant age categoires
nathmda_in %>%
  select(matches("age")) %>%
  filter(! applicant_age %in% c("8888", "9999") & ! co_applicant_age %in% c("8888", "9999") ) %>%
  ggplot() +
  geom_histogram(aes(applicant_age, fill = "blue"), alpha = .5, stat = "count") +
  geom_histogram(aes(co_applicant_age, fill = "orange"), alpha = .5, stat = "count")
```
