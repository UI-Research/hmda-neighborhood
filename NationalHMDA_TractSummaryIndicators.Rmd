...
nathmda_in %>% select(applicant_age, co_applicant_age) %>%
  mutate(
    across(c(applicant_age, co_applicant_age), 
    ~ case_when(
      . %in% c("8888", "9999") ~ NA_character_, ##these aren't defined in the documentation...
...
#merge(nathmda_states, counties, by.x="ucounty", by.y="ucounty", all.x=TRUE) %>% 
  mutate(
    ...
    geo2010 = case_when(
      ...
      TRUE ~ "99999999999")) %>%
  left_join(acs_housing, by = c("geo2010" = "GEOID"))
...
nathmda_flags <- left_join(nathmda_geo, il_in, by = "ucounty") %>%
  #merge(nathmda_geo, il_in, by.x = "ucounty", by.y = "ucounty", all.x = TRUE) %>%
    mutate(
      ...
      older_adult_flag = if_else(applicant_age == "65+" && co_applicant_age == "65+", 1, 0), ## need to include cases when there is no co_applicant (potentially the 8888 value?)
      middle_adult_flag = if_else(applicant_age %in% c("25-44", "45-64") && co_applicant_age %in% c("25-44", "45-64"), 1, 0), ## need to include cases when there is no co_applicant
      younger_adult_flag = if_else(applicant_age == "<25" && co_applicant_age == "<25", 1, 0), ## need to include cases when there is no co_applicant
