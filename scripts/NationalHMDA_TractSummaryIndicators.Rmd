...
library(skimr)
library(arrow)
library(tidylog, warn.conflicts = FALSE) # adding tidylog to check joins (TR, 12/5/2023)
```
...
#TR changed to 2022 for testing purposes, 12/5/2023
year = "2022"
...
#Path to subcounty-to-tract crosswalk from geocorr (.csv) 
geocorr_xwalk_path = here("data-raw", "geocorr_subcounty_tract_landarea_2018.csv")

## in cases where the ACS year would use 2020 Census tract geographies, instead
## use 2019 as the ACS data year
acs_year = year
if ((year %>% as.numeric) > 2019) { acs_year = "2019"}
...
hmda_raw <- read_parquet(file = hmda_path, col_select = all_of(hmda_columns)) %>%
  mutate(
    ...
    # note that 60 = American Samoa, 66 = Guam, 72 = PR, 78 = USVI
    county_code =  case_when(
      state_code %in% c("00", "03", "07", "80") ~ as.character(NA), 
      TRUE ~ as.character(county_code))) %>%
  ## For testing purposes only, uncomment this to run on a subset of all records
...
# replace_29055 = income_limits_raw %>%
#   filter(county_fips == "29055") %>%
#   group_by(county_fips) %>%
#   summarise(across(where(is.numeric), first)) %>%
#   mutate(county_count = 1, join_key = "29055")
# 
# income_limits_cleaned = income_limits_raw %>%
#   filter(county_fips != "29055") %>%
#   bind_rows(replace_29055)
...
tract_level_income_limits = geocorr_xwalk %>%
  ...
  filter(!is.na(census_tract)) %>% # uncomment pipe if testing on one specific state (TR, 12/5/2023)
  filter(str_sub(census_tract,1,2) =="01") # uncomment if testing on one specific state (TR, 12/5/2023)
```
...
#   unite(county_fips, state_code, county_code, sep = "", remove = F) %>%
#   # Dropping unneeded columns
#   select(state_name, county_fips)
...
## If and when HMDA data are updated to 2020 census tract vintages, this code will
## need to be adjusted (to use the most current 5-year ACS data); an updated version of 
## the tract-subcounty crosswalk will also be needed
...
hmda_geo <- hmda_raw %>% 
  mutate(
    ...
    geo2010 = case_when(
      ...
      missing_tract == 1 & missing_county == 0 ~ as.character(paste0(county_code, "000000")),
    ...
    ## retain them)
    geo2010 = case_when(
      state_code %in% c("66", "72", "78") ~ paste0(county_code, "999999"),
      T ~ geo2010))
...
hmda_flags <- hmda_geo %>%
    ...
    # Imputing with the county mean income limits when there's a valid county code but not a valid census code
...
hmda_combined <- hmda_flags %>%
  mutate(
    ...
    occupancy_investment_origination = create_var(occupancy_investment_flag, lien_firstlien_flag, outcome_originated_flag),
    
    #Occupancy - investment by unit size
    occupancy_investment_units_1_4 = create_var(occupancy_investment_flag, outcome_originated_flag, lien_firstlien_flag, units_1_4_flag),
    occupancy_investment_units_5ormore = create_var(occupancy_investment_flag, outcome_originated_flag, lien_firstlien_flag, units_5ormore_flag),
...
hmda_tract <- hmda_combined %>%
  ...
  left_join(
    ...
    acs_housing %>%
      ...
      #TR added on 12/6/2023
      rename(geo2010=GEOID),
  # by = c("geo2010"="GEOID") TR slightly modified the "by" command
    by = c(geo2010)) %>% # TR changed geo2010 to census_tract on 12/6/2023
...
```{r Data Quality Testing TR}
# load final outputted datasets
hmda_2018 <- read.csv("C:/Users/trajninger/Documents/TR/HMDA_Analysis/hmda-neighborhood/data-final/hmda_tract_2018.csv", header=TRUE)
hmda_2019 <- read.csv("C:/Users/trajninger/Documents/TR/HMDA_Analysis/hmda-neighborhood/data-final/hmda_tract_2019.csv", header=TRUE)
hmda_2020 <- read.csv("C:/Users/trajninger/Documents/TR/HMDA_Analysis/hmda-neighborhood/data-final/hmda_tract_2020.csv", header=TRUE)
hmda_2021 <- read.csv("C:/Users/trajninger/Documents/TR/HMDA_Analysis/hmda-neighborhood/data-final/hmda_tract_2021.csv", header=TRUE)
hmda_2022 <- read.csv("C:/Users/trajninger/Documents/TR/HMDA_Analysis/hmda-neighborhood/data-final/hmda_tract_2022.csv", header=TRUE)

options(scipen=999) # get rid of scientific notation

# spot checks on final outputted datasets
  View(skimr::skim(hmda_2018))
    # all variables are numeric
    # all variables have minimum of 0 except owner_loan_amount_median
    # all but 5 variables have 0 missing values
  
    # many variables have extreme outliers for the invalid geo (geo2010 == 99999999999). 
    # this is the case for all final datasets, hmda_2018-hmda_2022. should probably remove this row as it may be misleading to users.
      # is this row summing variable values for all missing geos?
      # ex: for 2018, owner_purchase_originations (originations for first-lien owner-occupied purchases of 1-4 unit dwellings) is 12,900 for geo 99999999999, next closest is 1623
      # ex: for 2018, occupancy_investment_origination (originations for loans made for the purpose of investment purchases). is 4628 for geo 99999999999, next closest is 429
  
  View(skimr::skim(hmda_2019))
  View(skimr::skim(hmda_2020))
  View(skimr::skim(hmda_2021))
  View(skimr::skim(hmda_2022))
  
    # the datasets for each year (2018-2022) have same issue with significant outliers for many variables for the geo2010 == 99999999999

# check code using subset of tracts in one state (Alabama)
  # adjusted line 129 to include: filter(state_code == "01"), which filters only the raw data to census tracts in Alabama   
  # look at hmda_raw and compare to output data (e.g., hmda_2022) for a few vars
    # hmda_2022 has 1493 unique census tracts, hmda_raw has 1427 unique census tracts
  # spot check raw data by comparing to calculations in final data for several census tracts. checked for variety of census tracts across the years.
  # check vars that were summed or where we took median
    # checked that, for several sample census tracts, the number of owner_purchase_originations was correct. compared output of below code with value in final dataset
      # ex code: sum(hmda_raw$census_tract=="01081041300" & hmda_raw$action_taken==1 & hmda_raw$loan_purpose==1 & hmda_raw$lien_status==1 & hmda_raw$occupancy_type==1, na.rm=TRUE) 
    # checked that occupancy_investment_origination was correct
      # ex code: sum(hmda_raw$census_tract=="01071950400" & hmda_raw$action_taken==1 & hmda_raw$loan_purpose==1 & hmda_raw$lien_status==1 & hmda_raw$occupancy_type==3, na.rm=TRUE)
    # checked that race_xxxx_purchase variables are correct
      # ex: for tract, 1071950400, there are 9 white purchases and 4 purchases with missing race data 
      # table(hmda_raw$applicant_race_1[hmda_raw$census_tract=="01071950400" & hmda_raw$action_taken==1 & hmda_raw$loan_purpose==1 & 
            # hmda_raw$lien_status==1 & hmda_raw$occupancy_type==1])
      # result is 9 for race 5 (white) and 4 for race 6 (information not provided by applicant)
      # correct because in final data, race_white_purchase=9 and race_missing_purchase=4
    # checked median income for several census tracts
      # ex: median(hmda_raw$income[hmda_raw$census_tract=="01081041300" & hmda_raw$action_taken==1 & hmda_raw$loan_purpose==1 & hmda_raw$lien_status==1 & hmda_raw$occupancy_type==1],
                  # na.rm=T)
    # checked owner_loan_amounts -- final data has medians
      # ex: median(hmda_raw$loan_amount[hmda_raw$census_tract=="01081041802" & hmda_raw$action_taken==1 & hmda_raw$loan_purpose==1 & hmda_raw$lien_status==1 &
                #  hmda_raw$occupancy_type==1], na.rm=T)
  
# check all joins
  # 1. full join of county-to-tract geocorr crosswalk with income limits data (line 215)
    # also need to subset tract_level_income_limits in line 231 to only Alabama
    # there are 67 counties in Alabama and 1179 census tracts
    # income limits in joined dataset (tract_level_income_limits) correspond to the specified income limits in income_limits_raw dataset for state of Alabama
    # full_join added 5 columns (county_fips, income_limit_very_low, income_limit_low, income_limit_moderate, county_count), both datasets had "census_tract" and "join_key" vars.
      # ended up removing 3 vars after the join: join_key, county_fips, county_count

  # 2. merge tract_level_income_limits with national raw dataset (hmda_geo) subsetted to only Alabama census tracts, where state code is 01 (line 319)
    # result: hmda_flags dataset, which includes flags for race, ethnicity, sex, age
    # added the 3 income limit vars from tract_level_income_limits
    # length(unique(hmda_geo$census_tract)); length(unique(hmda_flags$census_tract)); length(unique(tract_level_income_limits$census_tract))
      # same number of unique census tracts as there were in national raw dataset (hmda_geo): 1427
      # but the tract_level_income_limits dataset had different number of unique tracts: 1179 
        # makes sense because for census tracts without income limits data, we used the mean of county's income limits
  
  # 3. after creating a dataset with the combined indicators (hmda_combined), we join with the two ACS housing vars (line 659)
    # hmda_tract = joined dataset, turned back into tibble in line 683 (called hmda_final)
    # the join added 3 more vars (2 from acs_housing + one percent var that we created in code)
      # occupancy_owneroccupied_percent, occupancy_unitsoccupied_count, occupancy_owneroccupied_count
    # acs_housing dataset contributes 1181 unique census tract ids
    # hmda_combined contributes 1427 unique census tract ids
    # need to make sure that these 246 census tract ids that were in hmda_combined and not in acs_housing are coded as NAs
      # WRITE CODE FOR THIS

# do final outputs have missing data in the same place that ACS data
```
