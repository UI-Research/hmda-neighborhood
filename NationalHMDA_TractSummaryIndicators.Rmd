...
hmda_raw <- read_parquet(file = hmda_path, col_select = all_of(hmda_columns)) %>%
  mutate(
    ...
    #00, 03, 07, 72, and 80 are not state FIPS codes
    county_code =  case_when(
      state_code %in% c("00","03","07", "72", "80") ~ as.character(NA), 
      TRUE ~ as.character(county_code))) %>%
...
income_limits_raw <- read_csv(income_limits_path) %>%
  ...
  mutate(
    #Create padded state FIPS code - two characters
    state_code = str_pad(State, 2, pad = "0"),
    #Created padded county code - three characters
  ...
  ## county ("county count")
  mutate(join_key = if_else(county_count > 1, fips2010, county_fips)) %>%
...
geocorr_xwalk = read_csv(geocorr_xwalk_path) %>%
  ...
  mutate(
    ...
    join_key = if_else(county_fips %in% ( income_limits_cleaned %>% filter(county_count == 1) %>% pull(county_fips) ), county_fips, fips2010),
    join_key = case_when(
...
tract_level_income_limits = geocorr_xwalk %>%
  left_join(income_limits_cleaned, by = "join_key") %>%
  ...
  mutate(across(where(is.double), ~ if_else(is.na(.x), mean(.x, na.rm = T), .x))) %>%
  ungroup() %>%
...
# Join state names
hmda_states <- hmda_raw %>% left_join(states_tigris, by = c("county_code" = "county_fips"))
...
hmda_geo <- hmda_states %>%
  mutate(
    ...
    # But when state code is missing but census_tract is not, this is because it's a tract in PR, for which
    # we don't have income limit data--dropping these tracts
    county_code = case_when(
      is.na(county_code) & !is.na(census_tract) & is.na(state_code) & (str_sub(as.character(census_tract), 1, 5) %in% states_tigris$county_fips) ~ str_sub(as.character(census_tract), 1, 5),
      T ~ county_code),
...
hmda_flags <- left_join(hmda_geo, tract_level_income_limits, by = c("geo2010" = "census_tract")) %>%
...
if ( (nrow(failed_quality_checks) > 0) |  (nrow(codebook_variables_missing_from_dataset) > 0) | (length(dataset_variables_missing_from_codebook) > 0) ) { warning("Not all quality checks passed!") }
```
