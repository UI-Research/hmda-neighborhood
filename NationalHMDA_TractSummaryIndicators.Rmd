...
```{r converting the data to .parquet, eval = F}
#Location of raw HMDA data (1.49 GB zipped):
#https://s3.amazonaws.com/cfpb-hmda-public/prod/dynamic-data/2021/2021_lar.zip 

#Manually download this onto your local machine, then convert it to a .parquet file
#thereby cutting the file size to approximately 1.1 GB. Optionally, select relevant columns
#and only write these columns out to the .parquet file.
...
#Then, copy this file from your local machine to a folder on the F: drive on 
#Stata 2, or to another virtual desktop with good processing capacity
```
...
nathmda_geo %>% distinct(loan_type)
il_in
```
...
#Note: "_avail"-suffixed variables are used for quality control
nathmda_flags %>% select(income_high_flag, income, income)
...
nathmda_flags <- left_join(nathmda_geo, il_in, by = "ucounty") %>%
    mutate(
      ...
      #Income flags
      income_missing_flag = case_when(is.na(income) ~ 1, TRUE ~ 0), #Note - this is just the inverse of the income_avail flag...?
      ...
      income_available = if_else(!is.na(income), 1, 1, 0), #Note - this is just the inverse of the missing_income flag...?
...
#Check frequencies of new age categories
...
nathmda_comb1 <- nathmda_flags %>%
  ...
  mutate(#Top-line indicators
         owner_purchase = create_var(standard_flag), ## isn't this just the same as "standard_flag"?
         ...
         #Owner-occ purchasease loans by race
         ...
         #Owner-occ purchasease loans by specific race 
...
final_vars <- c(
  ...
  #Race/ethnicity vars
  "race_white_purchase","race_black_purchase","race_hispanic_purchase","race_asian_or_nhpi_purchase","race_multiple_purchase","race_missing_purchase","race_other_purchase", "race_mixed_purchase",
  "race_aian_purchase","race_asian_indian_purchase","race_asian_chinese_purchase","race_asian_filipino_purchase","race_asian_japanese_purchase","race_asian_korean_purchase",
  "race_asian_vietnamese_purchase",
...
hmda_tract <- nathmda_comb %>%
  ...
  left_join(
    acs_housing %>%
      ...
      select(GEOID, occupancy_owneroccupied_percent),
    by = c("geo2010" = "GEOID")
    ) %>%
...
hmda_final %>% 
  ...
  mutate(
    income_check = 
      income_verylow_purch +
      income_low_purch +
      income_moderate_purch +
      income_high_purch
  ) %>% 
  select(income_check, income_avail)
  ...
  select(race_avail, white_purch, black_purch, hisp_purch, api_purch, race_multiple_purch, race_mixed_purch, race_other_purch, race_check)


  hmda_final
  
hmda_final %>%
  select(race_avail, matches("purch")) %>%
  filter(race_avail == 1)
         white_purch = create_var(race_white_flag, standard_flag),
         black_purch = create_var(race_black_flag, standard_flag),
         hisp_purch = create_var(race_hispanic_flag, standard_flag),
         api_purch = create_var(api_flag, standard_flag),
         ais_purch = create_var(ais_flag, standard_flag),
         nhpi_purch = create_var(nhpi_flag, standard_flag),
         aian_purch = create_var(aian_flag, standard_flag),
         race_multiple_purch = create_var(race_multiple_flag, standard_flag),
         race_missing_purch = create_var(race_missing_flag, standard_flag),
         race_other_purch = create_var(race_other_flag, standard_flag),
  summarise(
    across(matches("avail"), ~ sum(.x, na.rm = T)),
    across(matches("age"))
  )
...
hmda_final %>%
  ...
  summarise(across(everything(), ~ sum(.x, na.rm = T))) %>%
  mutate(
    income_check_all = income_verylow_purch + income_low_purch + income_moderate_purch + income_high_purch, 
    income_check_white = rowSums(across(matches("white")))
  ) %>%
  select(income_avail, income_check_all, income_check_white)

hmda_final %>%
  select(race_avail, white_purch, black_purch, hisp_purch, api_purch, race_multiple_purch, race_mixed_purch, race_other_purch) %>%
  filter(race_avail > (white_purch + black_purch + hisp_purch + api_purch + race_multiple_purch + race_mixed_purch))
```
...
df_2021 %>% 
  select(matches("age")) %>%
  skim %>%
  write_csv(file = here("summary_statistics_2021_age_variables.csv"))
...
df_2021 %>% 
  ...
  mutate(total = sum(olderage_purch, middleyounger_age_purch, middleolder_age_purch, youngerage_purch, mixedage_purch))

## total age_purch values: 4,552,165

nrow(nathmda_in) # ~26 million
options(scipen = 999999)
df_2021 %>% skim

## some missingness, even for geos that are correct, likely due to suppression
df_2021 %>% filter(is.na(owner_occupied_perc)) %>% select(invalid_geo, geo2010, owner_occupied_perc)
