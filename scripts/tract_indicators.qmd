...
authors: "Rob Pitingolo, Will Curran-Groome, Tomi Rajninger"
format:
  html:
    toc: true
    code-fold: true
...
```{r Convert Raw Data to Parquet}
#| eval: false 

## if you're processing raw data files in .csv, .txt, or other delimited formats,
## configure the function template below appropriately and execute for each desired
## year of data
# 
# source(here("scripts", "raw_hmda_to_parquet.R"))
# 
# raw_hmda_to_parquet(
#   year = 2023,
#   inpath = here("data-raw", "2023_public_lar_csv.csv"),
#   delimit_character = ",")
```
...
codebook_path = here("UrbanInstitute_HMDA_neighborhood_data_codebook.xlsx")
...
income_limits_raw <- read_csv(income_limits_path) %>%
  janitor::clean_names() %>% 
  ## in some years, this is called "fips2010", in others just "fips"
  ## we rename to "fips" for consistency
  rename_with(.cols = matches("^fips"), .fn = ~ str_remove_all(.x, "[0-9]")) %>%
  transmute(
    state_code = str_sub(fips, 1, 2), ## two characters
    county_code = str_sub(fips, 3, 5), ## three characters
...
# Join national file and counties, flag and create dummy codes for missing tracts/counties
...
hmda_flags <- hmda_geo %>%
    ...
    # This does not help with tracts in PR, VI, and Guam because there's not any tracts with valid
    # income limits that can be ued for imputation
...
failed_quality_checks = hmda_final %>%
  ...
  { if (as.numeric(year) > 2021) mutate(., tract_id_characters = nchar(geo2020), geo = geo2020) else mutate(., tract_id_characters = nchar(geo2010), geo = geo2010) } %>%
  mutate(
    ...
    errors = case_when(
      ...
      (income_check != income_available) & (!str_detect(geo, "X"))  ~ "income check failed", ## the income limits don't map to these pseudo-identifiers; we exclude these tracts
      race_check != race_available ~ "race check failed",
      (race_income_check != race_income_available) & (!str_detect(geo, "X")) ~ "race-income check failed", ## the income limits don't map to these pseudo-identifiers; we exclude these tracts
      tract_id_characters != 11 ~ "invalid tract id",
      T ~ NA_character_)) %>%
  filter(!is.na(errors)) %>%
  select(errors, tract_id_characters, income_available, income_check,  matches("income"), -matches("race"), any_of(c("geo2010", "geo2020")))

## tracts that have invalid IDs (e.g., 97501XXXXX) have income_check values from above
## that don't match their income_available values. this is expected, but we want to ensure
## this doesn't affect too many observations
tracts_with_no_income_estimate_due_to_geography_missingness = hmda_final %>%
  # all tract IDs should be 11 characters
  { if (as.numeric(year) > 2021) mutate(., tract_id_characters = nchar(geo2020), geo = geo2020) else mutate(., tract_id_characters = nchar(geo2010), geo = geo2010) } %>%
  mutate(
    income_check = rowSums(across(c(matches("income"), -matches("race|median|available"))), na.rm = T),
    race_income_check = rowSums(across(c(matches("race.*income"), -matches("available|missing"))), na.rm = T),
    errors = case_when(
      (income_check != income_available)  ~ "income check failed", ## the income limits don't map to these pseudo-identifiers; we exclude these tracts
      (race_income_check != race_income_available)  ~ "race-income check failed", ## the income limits don't map to these pseudo-identifiers; we exclude these tracts
      tract_id_characters != 11 ~ "invalid tract id",
      T ~ NA_character_)) %>%
  filter(!is.na(errors))

if (nrow(tracts_with_no_income_estimate_due_to_geography_missingness > 100)) { 
  warning("More than 100 tracts have no income estimate due to geography missingness!") }
...
codebook_variables_missing_from_dataset = codebook %>%
  filter(
    !(`Field name` %in% (colnames(hmda_final))),
    ## in a given year, one of these will not be present in colnames(hmda_final)
    ## because the name is used to differentiated between the vintage of the census tract
    ## across years of hmda data
    !`Field name` %in% c("geo2010", "geo2020"))  
...
dataset_variables_missing_from_codebook = colnames(hmda_final)[!(colnames(hmda_final) %in% codebook$`Field name`)]
...
if ( (nrow(failed_quality_checks) > 0) |  (nrow(codebook_variables_missing_from_dataset) > 0) | (length(dataset_variables_missing_from_codebook) > 0) ) { 
  stop("Not all quality checks passed!") } else { 
